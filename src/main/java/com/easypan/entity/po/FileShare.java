...
public class FileShare implements Serializable {
	...
	public String toString() {
		return "分享ID:" + (shareId == null ? "空" : shareId) + ",文件ID:" + (fileId == null ? "空" : fileId) + ",分享人ID:" + (userId == null ? "空" : userId) + ",有效期类型 0:1天 1:7天 2:30天 3:永久有效:" + (validType == null ? "空" : validType) + ",失效时间:" + (expireTime == null ? "空" : DateUtils.format(expireTime,DateTimePatternEnum.YYYY_MM_DD_HH_MM_SS.getPattern())) + ",分享时间:" + (shareTime == null ? "空" : DateUtils.format(shareTime,DateTimePatternEnum.YYYY_MM_DD_HH_MM_SS.getPattern())) + ",提取码:" + (code == null ? "空" : code) + ",浏览次数:" + (showCount == null ? "空" : showCount);
	}
}